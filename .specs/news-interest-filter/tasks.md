...
Реализация двухэтапной обработки новостей: AI-оценка интересности + публикация в
человечном стиле. Новости с оценкой 4+ из 5 публикуются в Telegram с красивым
оформлением ссылок.

## Tasks

-
  1. [x] Настройка зависимостей
...
-
  3. [x] Создать модуль оценки интересности
  ...
  - [x] 3.3 Реализовать функцию evaluateEntry
    ...
    - System prompt с критериями оценки (актуальность, ценность, уникальность,
      масштаб)
    - Вычислять `shouldPublish` как `score >= 4`
    - _Requirements: 1.1, 1.2, 1.3, 1.4, 2.1_

-
  4. [x] Checkpoint - Проверка модуля evaluator
...
-
  6. [x] Интегрировать в main handler
...
-
  7. [x] Checkpoint - Проверка типов всего приложения
...
-
  9. [x] Final checkpoint - Полная проверка
...
- Модель `openai/gpt-5-mini` используется как для оценки, так и для написания —
  это снижает латентность
- Порог публикации (4 балла) захардкожен, но при необходимости можно вынести в
  env
